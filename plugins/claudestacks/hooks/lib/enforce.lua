-- The claudestacks rule-enforcement pipeline.
--
-- Reads the installed-plugins registry, keeps only claudestacks-marketplace plugins, loads each one's
...
M.MARKETPLACE_SUFFIX = "@claudestacks"
M.SENTINEL_PREFIX = "claudestacks-enforce-"
...
-- `{plugin_key = {record, ...}}` for @claudestacks plugins only.
--
...
function M.resolve(context, trace)
  ...
  note("registry: " .. #keys .. " @claudestacks plugin(s)")
  if #keys == 0 then
    note("STOP: no @claudestacks plugins in the registry")
    return {}
  end
