# claudestacks-guideline-rust

Rust engineering guidelines and a strict Definition-of-Done for Claude Code, delivered as a single
lazily-loaded skill: `claudestacks-guideline-rust:rust-guidelines`.

The skill loads only when you (or an agent) are editing Rust / Cargo files, so it costs no context
the rest of the time. The `claudestacks` plugin's execution agents invoke it to obtain the Rust DoD and
...
`claudestacks` plugin's dispatcher reads the manifest and surfaces this skill at
edit/design time. Installed without the `claudestacks` plugin it degrades to
...
## How it pairs with `claudestacks`

The main plugin's agents are language-agnostic; they look up the DoD and rules from whichever
`*-guidelines` skill is installed. Install this alongside `claudestacks` to give those agents a Rust
