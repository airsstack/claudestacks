...
9. **Hand off.** Invoke `claudestacks-sdd:write-plan` to convert the approved spec into an implementation plan. That is the only skill you invoke after this one and the only next step — do not write code or scaffold files yourself; the approved spec is the handoff artifact.
