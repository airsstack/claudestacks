...
This plugin is a member of the in-repository `claudestacks` marketplace and
depends on the `claudestacks` plugin.
...
/plugin install claudestacks@claudestacks        # dependency
...
- **`/claudestacks-journal:journal-setup`** explicitly provisions and force-rebuilds
  the index.
...
- `/claudestacks-journal:journal-capture` — write the **session story**
  (`sessions/session-<id8>.md`). Re-run anytime; each run **overwrites** the
  story from the transcript (pure function, no merge).
- `/claudestacks-journal:journal-note <topic>` — write/update one **atomic note**
...
- `/claudestacks-journal:journal-recall <query>` — spawns the isolated
...
- `/claudestacks-journal:journal-link <stem-or-topic>` — the same subagent in
  `link` mode: suggests existing notes to `[[link]]` from a subject note
  (subject excluded), reusing the recall machinery.
- `/claudestacks-journal:journal-helped <stem>` — confirms a recalled note aided
