---
name: execute-plan
description: Use when you have a written implementation plan to carry out — executes it task by task with review checkpoints, driving the claudestacks orchestrate skill per task and pausing for user review. Soft-coupled to the claudestacks main plugin; if that plugin is absent, degrades to guided inline execution.
---

# Execute Plan

Drive a written plan to a reviewed, presentable state, task by task. This skill does not design and does not plan — those are `claudestacks-sdd:brainstorm` and `claudestacks-sdd:write-plan`. It works with any plan in a documented on-disk format, whether written by `write-plan` or by hand.
...
Drive each task through `claudestacks:orchestrate`: it runs the coder → reviewer pipeline, handles the fix loop, and holds a per-task commit gate. Hand it one scoped task; it returns a reviewed result.

If it does not resolve — the `claudestacks` main plugin is not installed — degrade to **guided inline execution** on the main thread, applying the same discipline the plan specifies (test first, confirm it fails, implement, run every verification the plan names). Tell the user the agent pipeline was unavailable. Never fail hard for want of the main plugin.
...
2. Drive the implementation through `claudestacks:orchestrate` (or inline), giving it the task description, acceptance criteria, and named verifications.
