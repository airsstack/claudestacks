...
soft-couples to the `claudestacks` plugin for execution and degrades to guided inline work
...
with claudestacks-specific adjustments — see [Attribution](#attribution).
...
Skills are namespaced `claudestacks-sdd:<name>`.
...
| `execute-plan` | Plan → implementation with review checkpoints. Drives the `claudestacks` plugin's `orchestrate` skill when installed; degrades to guided inline execution otherwise. The user is the commit gate. |
...
- on demand with `/claudestacks-sdd:setup`,
...
plugin** (`superpowers@claude-plugins-official`). claudestacks-sdd is not a fork; it re-implements
...
- **Soft-coupling to the `claudestacks` plugin's `orchestrate`** skill for execution, degrading to
