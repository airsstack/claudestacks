//! Default `reqwest`-backed implementer of [`crate::transport::Transport`] (and, via the
//! blanket impl, [`crate::transport::HttpTransport`]).
...
use crate::transport::contract::Transport;
...
impl ReqwestTransport {
    ...
    /// Construct a transport with a default `clauders-transport/<version>` UA.
    ///
    /// # Errors
    /// Returns [`TransportError::Build`] when the underlying `reqwest::Client`
    /// cannot initialize (typically a TLS-backend load failure).
    pub fn try_new() -> Result<Self, TransportError> {
        Self::try_new_with_user_agent(concat!("clauders-transport/", env!("CARGO_PKG_VERSION")))
    }
