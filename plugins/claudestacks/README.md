...
`*-guidelines` skill you have installed (e.g. `claudestacks-guideline-rust`), and degrade gracefully
...
All components are namespaced `claudestacks:<name>`.
...
`plugins/claudestacks/scripts/install-airsl.sh` from the repo root (`scripts/install-airsl.sh` from
...
| --- | --- | --- |
| `/claudestacks:snapshot-save` | whole-session | Captures the session as a whole — the default "where was I" record. `topic:` left empty. |
| `/claudestacks:snapshot-save streaming parser` | topic-focused | Biases the summary, snippets, and carryovers toward *streaming parser* and stamps `topic: streaming parser` (also added to the `index.md` line). |
| `/claudestacks:snapshot-load` | current-branch | Loads the latest snapshot(s) for the **current branch** — branch orientation. |
| `/claudestacks:snapshot-load streaming parser` | topic search | Ranks **all** snapshots (any branch) by the topic — matching the saved `topic:` key first, then the `summary`. Branch only breaks ties. |
...
that thread back. Example: save `/claudestacks:snapshot-save retry backoff` on a spike branch; weeks
later, on `main`, `/claudestacks:snapshot-load retry backoff` rehydrates that thread without dragging in
...
The `claudestacks` plugin is the suite's single rule-enforcement dispatcher. A
...
claudestacks-marketplace plugins (keys ending `@claudestacks`), and loads each one's
...
Any claudestacks sub-plugin that enforces rules declares them in an
...
```json
{
  ...
  "detect": ["Cargo.toml"],
  "match": ["**/*.rs", "**/Cargo.toml"],
  "skill": "claudestacks-guideline-rust:rust-guidelines",
  "phase": ["code", "design"]
}
```
...
### Diagnosing it — `/claudestacks:enforce-doctor`
...
`/claudestacks:enforce-doctor <path>` runs `enforce.lua --explain <path>` — the same
