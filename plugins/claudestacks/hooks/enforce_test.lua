...
--     --allow-exec git plugins/claudestacks/hooks
--
...
local function plugins_root()
  ...
  for _ = 1, 10 do
    if fs.is_file(path.join(dir, "claudestacks", ".claude-plugin", "plugin.json")) then
      return dir
    end
...
return {
  a_matching_rust_file_emits_one_pointer = function()
    ...
    local pointers = resolve({
      ...
      cwd = project,
      registry = registry({ ["rust@claudestacks"] = { { installPath = install } } }),
    })
  ...
  a_record_bound_to_another_project_contributes_nothing = function()
    ...
    local file = registry({
      ["rust@claudestacks"] = { { installPath = install, projectPath = other } },
    })
  ...
  the_trace_names_the_silent_exit_that_was_taken = function()
    ...
    assert(table.concat(trace, "\n"):find("no @claudestacks plugins", 1, true),
      table.concat(trace, "\n"))
  end,
  ...
  no_plugin_driver_ever_emits_a_permission_decision = function()
    ...
    assert(root, "could not resolve the plugin suite root from this test file's own path "
      ...
      .. "`claudestacks/.claude-plugin/plugin.json`. This guard sweeps a checkout of this "
    ...
    assert(fs.is_file(path.join(root, "claudestacks", "hooks", "enforce.lua")),
      "resolved the suite root to " .. root .. ", which holds no claudestacks/hooks/enforce.lua")
