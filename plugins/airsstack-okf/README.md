...
(`cargo install airsl-cli --locked`), which the scripts run on. No Python, no jq/yq; agents use
only harness built-ins (Read/Glob/Grep, with Grep backed by Claude Code's bundled ripgrep).
