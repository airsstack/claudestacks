...
- **Honor the active stack's guidelines.** Detect the stack from repo markers (`Cargo.toml` → Rust) and
  load the matching guideline skill (`claudestacks-guideline-rust:rust-guidelines`). Every code block you
...
hook or `/claudestacks-sdd:setup` has run.

Two execution paths:

1. **Subagent-driven.** A fresh subagent per task via `claudestacks-sdd:execute-plan`. Review each receipt
